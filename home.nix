...

{
  ...
  programs.bash = {
    enable = true;
    shellAliases = {
      nixos = "cd ~/dotfiles && git add . && (git commit -m \"Update $(date)\" || true) && git push origin main && git add -f -N nixos/hardware-configuration.nix && sudo nixos-rebuild switch --flake .#naxce";
    };
  };
