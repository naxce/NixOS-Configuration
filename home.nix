...

{
  ...
  programs.bash = {
    enable = true;
    shellAliases = {
      nixos = "cd ~/dotfiles && git add . && (git commit -m \"Update $(date)\" || true) && git push origin main && git add -N -f nixos/hardware-configuration.nix && sudo nixos-rebuild switch --flake .#naxce";
    };
  };
